...
Most of these designs are already documented across multiple sources and referenced accordingly.
...
> In the following 1.1 and 1.2 designs, the ARS **only** purpose is to push the default route to AVS.
...
> Make sure to disable GW route propagation on the internet facing NIC of the FW NVA, to avoid a routing loop.
...
> Here the role of the ARS is to both push the default route to AVS and enable the On-Prem <=> AVS transit.
...
> Internet connectivity can be provided either by the FW NIC facing the AVS NVA or a 3rd dedicated NIC on the FW NVA. GW route propagation should be disabled for that NIC.
...
> If stateful, the NVA instances should be configured as Active/Standby to avoid asymmetric routing. 
